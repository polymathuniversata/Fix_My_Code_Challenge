...
class User():
    ...
    def password(self, pwd):
        ...
        if pwd is None or type(pwd) is not str:
            self.__password = None  # Corrected from self._password to self.__password
        else:
    ...
    def is_valid_password(self, pwd):
        ...
        if self.__password is None:
            return False
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password  # Convert hashed input password to lowercase
